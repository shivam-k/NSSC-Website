...
<?php
if(isset($_POST['aero'])) {
   $check = "SELECT * FROM workshops WHERE pi = '$pi_admin'";
   $result = mysqli_query($dbcon,$check);
   if(mysqli_num_rows($result) > 0) {  // pi is already indatabase
      while($row = mysqli_fetch_assoc($result)) {
        $aero = $row['aero'];
      }
      if(empty($aero) ) { // already enrolled
          $update = "UPDATE workshops SET aero = '1'";
          if(mysqli_query($dbcon, $update) > 0) {
              echo "<script>alert('Successfuly enroled !!')</script>";
          }
          else {
              echo "<script>alert('Problem in updating row.....$pi_admin')</script>";
          }
      }
      else { // already enrolled in this workshop
           echo "<script>alert('You are already enrolled in this Workshop')</script>";
      }
   }
  else { // pi is not in database
    $insert = "INSERT INTO workshops (pi, aero, hawk, astro, agra) VALUE ('$pi_admin', '1', '', '', '')";
    if(mysqli_query($dbcon, $insert) > 0) {
       // echo "<script>alert('successfuly inserted row...$pi_admin')</script>";
       echo "<script>alert('Successfuly enroled.. !!')</script>";
    }
  }
}
...
?>
        ...
        <form method="POST" action="" style="">
            <input type="submit" name="aero" value="Click to Enroll" style="cursor: pointer;display: table; margin: 0 auto" />
        </form>
...
if(isset($_POST['hawk'])) {
   ...
   if(mysqli_num_rows($result) > 0) {  // pi is already indatabase
      ...
      if(empty($hawk)) { // already enrolled
          $update = "UPDATE workshops SET hawk = '1'";
          if(mysqli_query($dbcon, $update) > 0) {
              echo "<script>alert('Successfuly enroled !!')</script>";
          }
          else {
...
?>
        ...
        <form method="POST" action="" style="">
            <input type="submit" name="hawk" value="Click to Enroll" style="cursor: pointer;display: table; margin: 0 auto" />
        </form>
<br />
        <h2 style="text-align: center">CONTACTS</h2>
...
if(isset($_POST['astro'])) {
   ...
   if(mysqli_num_rows($result) > 0) {  // pi is already indatabase
      ...
      if(empty($astro) ) { // already enrolled
          $update = "UPDATE workshops SET astro = '1'";
          if(mysqli_query($dbcon, $update) > 0) {
              echo "<script>alert('Successfuly enroled !!')</script>";
          }
          else {
...
?>
        ...
        <form method="POST" action="" style="">
            <input type="submit" name="astro" value="Click to Enroll" style="cursor: pointer;display: table; margin: 0 auto" />
        </form>
<br />
        <h2 style="text-align: center">CONTACTS</h2>
...
if(isset($_POST['agra'])) {
   ...
   if(mysqli_num_rows($result) > 0) {  // pi is already indatabase
      ...
      if(empty($agra) ) { // already enrolled
          $update = "UPDATE workshops SET agra = '1'";
          if(mysqli_query($dbcon, $update) > 0) {
              echo "<script>alert('Successfuly enroled !!')</script>";
          }
          else {
...
?>
        ...
        <form method="POST" action="" style="">
            <input type="submit" name="agra" value="Click to Enroll" style="cursor: pointer;display: table; margin: 0 auto" />
        </form>
<br />
        <h2 style="text-align: center">CONTACTS</h2>
